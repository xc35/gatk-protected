...
public final class ContEst extends LocusWalker {
    ...
    public void apply(final AlignmentContext context, final ReferenceContext ref, final FeatureContext features) {
        ...
        final ReadPileup defaultPile = useSequencingGenotypes ? context.getBasePileup().getPileupForSample(evalSample, getHeaderForReads()) :
                ; context.getBasePileup();
    ...
    private Genotype getGenotype(final FeatureContext features, final AlignmentContext context, final ReferenceContext referenceContext, final boolean useSeq) {
        if (!useSeq) {
            final Genotype g = getGenotypeFromArray(features, genotypes, verifiedSampleName, verifySample, sampleName);
            if (g != null) {
     ...
     * @param precision the precision we're aiming for
    ...
    private static Map<String, ContaminationStats> calcStats(final byte referenceBase,
        ...
        for (final String pop : pops) {
            ...
            if (myAllele == info.minorAllele) {
                ...
                if (pops.length == 1 && baseReport != null) {
                    baseReport.print(
                            StringUtil.join("\t",
                                    lane,
                                    loc.getContig(),
    ...
    public Object onTraversalSuccess() {
        ...
        for (final Map.Entry<String, Map<String, ContaminationStats>> entry : accumulatedResult.getStats().entrySet()) {
            ...
            if (!newMap.isEmpty()) {
                ...
                logger.info("Warning: We're throwing out lane " + entry.getKey() + " since it has fewer than " + minBaseCount +
        ...
        accumulatedResult.outputReport(precision, TRIM_FRACTION, TRIM_INTERVAL, BETA_THRESHOLD);
