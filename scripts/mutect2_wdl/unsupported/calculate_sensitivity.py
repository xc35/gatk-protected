...
def generate_sensitivity_table(df, type):
    ...
    df.loc[:, 'af_bin'] = df[EXPECTED_AF_COLUMN].apply(assign_snp_af_bin if type == "SNP" else assign_indel_af_bin)
    ...
    sensitivity = called.values / total.values
    ...
    sensitivity_with_confidence.to_csv(type + "_sensitivity.tsv", sep = '\t')

    return sensitivity_with_confidence, af_bins

def draw_sensitivity_graph(df, af_bins, type):
    x = df.index    # depth
    ...
    plt.legend(loc = 4)
    plt.title(type + " Sensitivity")
    plt.xlabel('Depth')
    plt.ylabel('Sensitivity')
    plt.savefig(type + "_sensitivity.png")
